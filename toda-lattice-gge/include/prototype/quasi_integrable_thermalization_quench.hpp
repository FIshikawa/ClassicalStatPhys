...
int main(int argc, char **argv){
  ...
  if(process_id == 0){
    ...
    dataput << "Declare Configulation " << std::endl 
            << "Explore Non-thermal Fixed Point : Quench Dynamics" << std::endl
            << "Frist Hamiltonian for relaxation : " << FirstHamiltonian::name() << std::endl
  ...
  for(const auto& key  : physical_value_list){
    ...
    if(process_id == 0) for(int i = 0 ; i < N_total_data; ++i) 
      results["error_" + key][i] = (N_loop > 1 ) ? std::sqrt((reduced_vector[i] / (N_loop-1) 
            - results[key][i] * results[key][i] * (N_loop)/(N_loop-1) ) / N_loop) : 0;
  }
