...
//  }
  ...
  results_2d["TodaConservationsSum1"] = std::vector< std::vector<double> >(N_total_data, std::vector<double>(num_particles,0.0));
  results_2d["TodaConservationsSum2"] = std::vector< std::vector<double> >(N_total_data, std::vector<double>(num_particles,0.0));
  results_2d["TodaConservationsSum3"] = std::vector< std::vector<double> >(N_total_data, std::vector<double>(num_particles,0.0));
  results_2d["TodaConservationsSum4"] = std::vector< std::vector<double> >(N_total_data, std::vector<double>(num_particles,0.0));
  results_2d["TodaConservationsCount"] = std::vector< std::vector<double> >(N_total_data, std::vector<double>(num_particles,0.0));
  results_2d["TodaConservationsVariance"] = std::vector< std::vector<double> >(N_total_data, std::vector<double>(num_particles,0.0));
  results_2d["error_TodaConservationsVariance"] = std::vector< std::vector<double> >(N_total_data, std::vector<double>(num_particles,0.0));
...
//  }
  ...
  for(int step = 0 ; step < N_total_data; ++step){
    for(int i = 0 ; i < num_particles; ++i){
      ...
      results_2d["TodaConservationsSum1"][step][i] = toda_conservations[step][i].sum1(); 
      results_2d["TodaConservationsSum2"][step][i] = toda_conservations[step][i].sum2(); 
      results_2d["TodaConservationsSum3"][step][i] = toda_conservations[step][i].sum3(); 
      results_2d["TodaConservationsSum4"][step][i] = toda_conservations[step][i].sum4(); 
      results_2d["TodaConservationsCount"][step][i] = static_cast<double>(toda_conservations[step][i].count()); 
...
//    }
  ...
  for(int step = 0 ; step < N_total_data; ++step){
    // NormalMode
    for(std::string value_name: {"Normalmode_Energy","TodaEigenvalues"}){
    ...
    for(std::string value_name: {"TodaConservationsSum1","TodaConservationsSum2","TodaConservationsSum3","TodaConservationsCount"}){
      mpi_error = MPI_Reduce(&results_2d[value_name][step][0], &reduced_vector_1[0], num_particles, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      if(process_id == 0) for(int i = 0 ; i < num_particles; ++i) results_2d[value_name][step][i] = reduced_vector_1[i];
    }
...
//    dataput.time_tag() << "end correlation values calc" << std::endl;
    ...
    for(int step = 0 ; step < N_total_data ; ++step){
      toda_conservations[step] = std::vector<tools::Accumulator>(num_particles);
      for(int i = 0; i < num_particles ; ++i){
        toda_conservations[step][i].reset();
        toda_conservations[step][i].add(
                                       results_2d["TodaConservationsSum1"][step][i],
                                       results_2d["TodaConservationsSum2"][step][i],
                                       results_2d["TodaConservationsSum3"][step][i],
                                       results_2d["TodaConservationsSum4"][step][i],
                                       static_cast<long>(results_2d["TodaConservationsCount"][step][i])
                                       );
        results_2d["TodaConservations"][step][i] = toda_conservations[step][i].mean();
        results_2d["error_TodaConservations"][step][i] = toda_conservations[step][i].error();
        results_2d["TodaConservationsVariance"][step][i] = toda_conservations[step][i].variance();
        results_2d["error_TodaConservationsVariance"][step][i] = toda_conservations[step][i].variance_error();
      }
    }
