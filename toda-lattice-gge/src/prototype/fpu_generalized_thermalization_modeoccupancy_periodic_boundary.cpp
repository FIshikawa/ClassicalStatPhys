...
inline void ParameterSet(int argc, char **argv, Parameters & parameters, int & input_counter){
  ...
  double diff = 1.0 - parameters.beta /( parameters.alpha * parameters.alpha * 2 / 3);
  if(std::abs(diff) < 1e-4)
    parameters.beta = parameters.alpha * parameters.alpha * 2 / 3;
  diff = 1.0 - parameters.gamma / (parameters.alpha * parameters.alpha * parameters.alpha / 3);
  if(std::abs(diff) < 1e-4)
    parameters.gamma = parameters.alpha * parameters.alpha * parameters.alpha / 3;
  diff = 1.0 - parameters.delta / (parameters.alpha * parameters.alpha * parameters.alpha * parameters.alpha * 2 / 15);
  if(std::abs(diff) < 1e-4)
    parameters.delta = parameters.alpha * parameters.alpha * parameters.alpha * parameters.alpha * 2 / 15;
}
