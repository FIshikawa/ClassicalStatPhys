...
#ifdef LATTICE_FULLYCONNECTED_HPP
#include <physics/classical_xy_external_fullyconnected.hpp>
using Hamiltonian =  hamiltonian::ClassicalXYExternalFullyConnected;
#else 
#include <physics/classical_xy_external.hpp>
using Hamiltonian =  hamiltonian::ClassicalXYExternal;
#endif
...
using Integrator =  integrator::DissipatedRungeKutta;
...
int main(int argc, char **argv){
  ...
  int Ns_observe = 3;
  double D = 1.0; //Intencity of isotropic External Force
  double Inte = 1.0; //Intencity of External Force
  double Freq = 1.0; //Frequency of External Force
 ...
 //parameter set 
  ...
  if (argc > input_counter) D =                boost::lexical_cast<double>(argv[input_counter]);++input_counter;
  if (argc > input_counter) Inte =             boost::lexical_cast<double>(argv[input_counter]);++input_counter;
  if (argc > input_counter) Freq =             boost::lexical_cast<double>(argv[input_counter]);++input_counter;

 // parameter auto set
  int N_each = N_loop/N_parallel;
  int num_particles = Lattice::set_num_particles(Ns);
  double dt = t/(double)N_time;
 ...
 //parameter set 
  ...
  dataput << "Declare Configulation " << std::endl 
          << "Density:Non Equilibrium Dynamics in classical XY with periodical external field" << std::endl
 ...
 //condition declare
  dataput << "Coupling constant: J is " << J << std::endl
          ...
          << "Strength of the dissipation : gamma  = " << gamma  << std::endl
          << "Intencity of the external field : Inte = " << Inte << std::endl
          << "Frequency of the external field : Freq = " << Freq << std::endl
          << "Intencity of the istropic external field : D = " << D << std::endl
  ...
  std::unordered_map<std::string, std::vector< std::vector<double> > > hist_data; 
 	hist_data["Velocity"] = std::vector< std::vector<double> >(N_time, std::vector<double>(N_loop)),
 	hist_data["Spin"] = std::vector< std::vector<double> >(N_time, std::vector<double>(N_loop));
  ...
  Hamiltonian hamiltonian(num_particles,J,pair_table,N_adj,Inte,Freq,D); 
  Integrator integrator(T, gamma, 2*num_particles);
  ...
  for(int c_loop=0; c_loop < N_each; ++c_loop){
    ...
    for(int step = 0; step < N_time; ++step){  
      ...
      pt += dt;
      integrator.step(pt, dt, z, hamiltonian, mt);
  ...
  //dataput.output_result_3d(correlation_dat,domain_correlation,);
