...
#include <clstatphys/integrator/velocity_velret.hpp>

namespace {
class VelocityVelretTest: public ::testing::Test {
  protected:  
  virtual void SetUp(){
...
TEST_F(VelocityVelretTest, TimeDevelopTest) {
  hamiltonian::HarmonicOscillator hamiltonian(num_particles,J,pair_table,N_adj);
  // set integrator 
  integrator::VelocityVelret integrator(2*num_particles);
  double dt = 1.0e-2;
  double pt = 0.0;
  double N_time = 1e+4;
  double error = dt * dt * dt * 10; //10 is assumed constant of error 
