...
#include <clstatphys/integrator/yoshida_4th_parallel.hpp>

namespace {
class Yoshida4thParallelTest: public ::testing::Test {
  protected:  
  virtual void SetUp(){
...
TEST_F(Yoshida4thParallelTest, TimeDevelopTest) {
  hamiltonian::HarmonicOscillator hamiltonian(num_particles,J,pair_table,N_adj);
  // set integrator 
  integrator::Yoshida4thParallel integrator(2*num_particles);
  double dt = 1.0e-2;
  double pt = 0.0;
  double N_time = 1e+4;
