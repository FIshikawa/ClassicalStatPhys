...
#include <clstatphys/physics/fpu_generalized_fixed_end_parallel.hpp>
#include <clstatphys/lattice/chain_open_boundary.hpp>
#include <clstatphys/integrator/runge_kutta_4th.hpp>

namespace {
class FPUGeneralizedFixedParallelEndTest: public ::testing::Test {
  protected:  
  virtual void SetUp(){
...
TEST_F(FPUGeneralizedFixedParallelEndTest, EnergyTest) {
  hamiltonian::FPUGeneralizedFixedEndParallel hamiltonian(num_particles,J,alpha,beta,gamma,delta,pair_table,N_adj);
...
TEST_F(FPUGeneralizedFixedParallelEndTest, ForceTest) {
  hamiltonian::FPUGeneralizedFixedEndParallel hamiltonian(num_particles,J,alpha,beta,gamma,delta,pair_table,N_adj);
  // set integrator 
  ...
  double pt = 0.0;
  double N_time = 1e+4;
  double error_rungekutta = dt * dt * dt * dt * dt * 1e+3; //10 is assumed constant of error 
