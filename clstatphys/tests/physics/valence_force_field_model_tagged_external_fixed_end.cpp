...
#include <clstatphys/physics/valence_force_field_model_tagged_external_fixed_end.hpp>
#include <clstatphys/lattice/chain_open_boundary.hpp>
#include <clstatphys/integrator/runge_kutta_4th.hpp>

namespace {
class ValenceForceFieldModelTaggedExternalFixedEndTest: public ::testing::Test {
  protected:  
  virtual void SetUp(){
    ...
    expected_energy_potential =
      ((J * 9 + 2 * alpha / 8 + beta * 1) 
      + (J * 4 + 2 * alpha / 8 + beta * 1)) * (num_particles-1)/2 
      + J * 9 + 2 * alpha / 8 + beta * 1
      - std::sin(0.5 * M_PI);
    expected_energy_total = expected_energy_kinetic + expected_energy_potential;
  }

  int  num_particles, N_adj, observe;
  double J,alpha, beta,frequency,intensity,phase;
  double expected_energy_potential, expected_energy_kinetic, expected_energy_total;
  std::vector<std::vector<int> > pair_table;
  std::vector<double> z;
};

TEST_F(ValenceForceFieldModelTaggedExternalFixedEndTest, EnergyTest) {
  hamiltonian::ValenceForceFieldModelTaggedExternalFixedEnd 
    hamiltonian(num_particles,J,alpha,beta,observe,intensity,frequency,phase,pair_table,N_adj);
...
TEST_F(ValenceForceFieldModelTaggedExternalFixedEndTest, ForceTest) {
  hamiltonian::ValenceForceFieldModelTaggedExternalFixedEnd 
    hamiltonian(num_particles,J,alpha,beta,observe,intensity,frequency,phase,pair_table,N_adj);
  // set integrator 
  ...
  double pt = 0.0;
  double N_time = 1e+5;
  double error_rungekutta = dt * dt * dt * dt * dt * 1000; //10 is assumed constant of error 
  ...
  EXPECT_FALSE(energy_init == hamiltonian.energy(pt,z));
  EXPECT_NEAR(energy_init/hamiltonian.energy(pt,z),1.0,error_rungekutta*2*N_time);
  EXPECT_FALSE(potential_init == hamiltonian.potential_energy(pt,z)); 
  EXPECT_FALSE(kinetic_init == hamiltonian.kinetic_energy(pt,z)); 
}
