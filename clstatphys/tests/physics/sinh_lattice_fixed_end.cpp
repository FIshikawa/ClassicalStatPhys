...
#include <clstatphys/physics/sinh_lattice_fixed_end.hpp>
#include <clstatphys/lattice/chain_open_boundary.hpp>
#include <clstatphys/integrator/runge_kutta_4th.hpp>

namespace {
class SinhLatticeFixedEndTest: public ::testing::Test {
  protected:  
  virtual void SetUp(){
...
TEST_F(SinhLatticeFixedEndTest, EnergyTest) {
  hamiltonian::SinhLatticeFixedEnd hamiltonian(num_particles,J,alpha,pair_table,N_adj);
...
TEST_F(SinhLatticeFixedEndTest, ForceTest) {
  hamiltonian::SinhLatticeFixedEnd hamiltonian(num_particles,J,alpha,pair_table,N_adj);
  // set integrator 
